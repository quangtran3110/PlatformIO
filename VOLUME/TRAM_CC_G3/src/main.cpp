...
#define BLYNK_AUTH_TOKEN "DEdOyQWTbvQ5_ma_MEP1_Z8gefY_rnfE"

#define BLYNK_FIRMWARE_VERSION "260907"
...
#define URL_fw_Bin "https://raw.githubusercontent.com/quangtran3110/PlatformIO/main/VOLUME/TRAM_CC_G3/.pio/build/nodemcuv2/firmware.bin"
...
constexpr uint32_t STATE_MAGIC = 0x43334451UL; // "C3DQ"
...
bool initializeStorage() {
  ...
  Serial.printf("EEPROM: san sang, pending=%u, pulse=%lu\n", state.queueCount,
...
bool buildDailyUploadBody(String &body) {
  ...
  for (uint8_t index = 0; index < state.queueCount; index++) {
    ...
    DailyRecord record = {};
    if (!readDailyRecord(slot, record)) {
      Serial.printf("V58: ban ghi EEPROM loi tai slot %u\n", slot);
      return false;
    }
...
bool uploadQueuedDailyRecords() {
  ...
  if (statusCode == HTTP_CODE_OK && response == "OK") {
    Serial.printf("V58: da gui %u ban ghi timestamped\n", state.queueCount);
    return true;
  }

  Serial.printf("V58: HTTP %d, response=%s\n", statusCode, response.c_str());
  return false;
}
...
bool enqueueClosedDay(uint32_t newDayKey) {
  ...
  if (state.queueCount >= DAILY_QUEUE_CAPACITY) {
    if (!queueFullReported) {
      ...
      Serial.println(F("V58: hang doi day, chua the chot ngay"));
    }
    return false;
  }
...
void serviceDailyUpload() {
  ...
  if (!persistState()) {
    state = beforeAck;
    nextDailyUploadMs = nowMs + dailyRetryMs;
    Serial.println(F("EEPROM: chua luu duoc xac nhan V58, se gui lai"));
    return;
  }
