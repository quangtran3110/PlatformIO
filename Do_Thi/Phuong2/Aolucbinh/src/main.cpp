...
bool sta_rl1 = LOW, sta_rl2 = LOW, prev_sta_rl1 = LOW;
String num_van;
char s_day[50] = "";
char B[50] = "";
...
void print_terminal()
{
  ...
  server_path = server_name + "batch/update?token=" + Main_TOKEN + pin_terminal + location + pin_terminal + s_weekday + pin_terminal + s_timer_van_1 + pin_terminal + s_timer_van_2;
  http.begin(client, server_path.c_str());
...
BLYNK_WRITE(V0)
{
  ...
  else if (dataS == "van1_on")
  { // RL1 on
    if (data.mode == 0)
      on_van1();
  }
  else if (dataS == "van1_off")
  { // RL1 off
    if (data.mode == 0)
      off_van1();
  }
  ...
  else if (dataS == "van2_on")
  { // RL2 on
    if (data.mode == 0)
      on_van2();
  }
  else if (dataS == "van2_off")
  { // RL2 off
    if (data.mode == 0)
      off_van2();
  }
