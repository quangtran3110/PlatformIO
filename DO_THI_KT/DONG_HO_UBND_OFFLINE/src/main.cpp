...
const int SYNC_HOUR = 2;    // Giờ thực hiện đồng bộ (2 giờ sáng)
const int SYNC_MINUTE = 0;  // Phút thực hiện đồng bộ
bool dailySyncDone = false; // Cờ báo đã đồng bộ trong ngày hôm nay hay chưa

unsigned long lastAdjust = 0;
unsigned long syncStartTime = 0;                  // Thời điểm bắt đầu quá trình đồng bộ
const unsigned long SYNC_TIMEOUT = 5 * 60 * 1000; // 5 phút timeout cho việc đồng bộ
bool syncCompletedSuccessfully = false;           // Cờ báo đồng bộ thành công
bool rtcSyncRequested = false;                    // Cờ báo yêu cầu đồng bộ RTC
//-------------------
...
void processRtcSync() {
  ...
  if (rtcSyncRequested && year() > 2023) {
    Serial.println("Processing RTC sync request...");

    // 1. Lấy thời gian hiện tại từ server Blynk. WidgetRTC đã tự động điều chỉnh theo múi giờ của project.
    DateTime blynkLocalTime(year(), month(), day(), hour(), minute(), second());

    // 2. Cập nhật RTC trực tiếp với thời gian đã nhận được
    rtc_module.adjust(blynkLocalTime);
    Serial.println("RTC time synced with Blynk server.");
    print_clock_time("Thời gian đã đồng bộ: ", blynkLocalTime.unixtime());
...
void handleOfflineMode() {
  ...
  if (now.hour() == SYNC_HOUR && now.minute() >= SYNC_MINUTE && !dailySyncDone) {
    ...
    syncStartTime = millis();          // Bắt đầu đếm giờ cho timeout
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(SENSOR_1_PIN), sensor_1, FALLING);
  attachInterrupt(digitalPinToInterrupt(SENSOR_2_PIN), sensor_2, FALLING);
  attachInterrupt(digitalPinToInterrupt(SENSOR_3_PIN), sensor_3, FALLING);
  attachInterrupt(digitalPinToInterrupt(SENSOR_4_PIN), sensor_4, FALLING);
